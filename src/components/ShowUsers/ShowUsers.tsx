...
const ShowUsers: React.FC = () => {
  ...
  const getUsers = async () => {
    try {
      const response = await apiInstance.get<{ users: User[] }>(`${apiBackend}/api/v1/auth/users`);
  ...
  const handleOk = async () => {
    try {
      ...
      const response = await apiInstance.put(`${apiBackend}/api/v1/auth/user/${currentUser.id}`, updatedUser);
